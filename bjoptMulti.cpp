...
pair<int, double> simulate(int start, int end, int &fails, int &hands);
...
int main(){
        ...
        for (int s5 = minBet; s5 <= s6; s5++){
            for (int s4 = minBet; s4 <= s5; s4++){
                for (int s3 = minBet; s3 <= s4; s3++){
                    for (int s2 = minBet; s2 <= s3; s2++){
                        for (int s1 = minBet; s1 <= s2; s1++){
                            cout << "\rSpread " << ++currentSpread << " of " << numSpread << flush;
                            ...
                            for (unsigned int i = 0; i < numThreads; i++){
                                ...
                                threads.emplace_back([start, end, &fails, &hands](){
                                    int threadFails = 0;
                                    int threadHands = 0;
                                    auto result = simulate(start, end, threadFails, threadHands);
                                    {
    ...
    if (possible){
        ...
        cout << " average per hand:" << avg << endl;
        cout << "Risk of Ruin:" << (safeFails/iterations)*100 << "%" << endl;
    }
}
void shuffle(){
    si = 0;
...
pair<int, double> simulate(int start, int end, int &fs, int &hs){
    int sHands = 0;
    int ret1 = 0;//number of Hands played
